...
const usage = `Usage:
  node tools/analyzers/create-ffmpeg-scene-manifest.mjs \\
...
async function main() {
  ...
  const adapterPath = await file(
    fileURLToPath(new URL("./ffmpeg-scene-worker.mjs", import.meta.url)),
  );
  ...
  const manifest = {
    ...
    name: "Audited FFmpeg scene detection",
    description: "Timestamped shot/scene ranges from FFmpeg scene scores",
    outputTypes: ["scenes"],
    assetKinds: ["video"],
    deterministic: true,
    parameterSchema: {
    ...
    metadata: {
      adapter: "frameos.ffmpeg-scene-worker",
      upstream: "https://ffmpeg.org/ffmpeg-filters.html#select_002c-aselect",
    },
  };
